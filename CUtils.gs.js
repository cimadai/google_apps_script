/**
 * Slackへ投稿。
 * @param {string} slackToken Slackのトークン
 * @param {string} botName ボット名
 * @param {string} botIconUrl ボットの画像URL
 * @param {string} channelId 発言先のチャネルID
 * @param {string} message 発言メッセージ
 */
function postToSlack(slackToken, botName, botIconUrl, channelId, message) {
  var slackApp = SlackApp.create(slackToken);
  slackApp.chatPostMessage(channelId, message, { username : botName, icon_url : botIconUrl});
}


/**
 * 指定された時刻のyyyy/mm/ddを取得。
 * 区切り文字はseparatorで指定可能。
 * @param {Date} dt Date
 * @param {string} separator 区切り文字
 * @return {string} yyyy${sep}mm${sep}dd
 */
...
/**
 * 指定された時刻のyyyy/mm/dd hh:MM:00を取得。
 * 区切り文字は上記で固定。秒も00で固定。
 * @param {Date} dt Date
...
function getDateTimeString(dt) {
    ...
    return getDateString(dt, "/") + " " + h + ":" + m + ":00";
}

/**
 * @example: 
 *      var cron = '0 10 * * ? *'; // 10 am in Japan (UTC+9)
 * 時間は1分間隔の実行。
 * @param {string} cron cron文字列
 * @param {function} callback コールバック関数
 */
function executeAt(cron, callback) {
    var s = Later.later.parse.cron(cron);
    var dt = new Date();
    if (dt.getDay() == 0 || dt.getDay() == 6) {
        // skip weekend. 0: SUN, 6: SAT
        Logger.log("Skip because weekend." + " day = [" + dt.getDay() + "]");
        return;
    }

    var dateKey = getDateString(dt, "/");
    var holidays = getHolidays(dt.getFullYear());
    if (holidays[dateKey] != undefined) {
        // skip holiday
        Logger.log("Skip because holiday." + " holiday = [" + holidays[dateKey] + "]");
        return;
    }
    var min1 = 60 * 1000;
    if ((dt.getTime() - Later.later.schedule(s).prev(1).getTime()) < min1) {
      callback(dt);
    }
}


// http://qiita.com/hrdaya/items/34ac4ab06a97debb1ffc
/**
...
function getHolidays(year) {
    ...
    var holidays = [
        ...
        [2017, 9999, 12, 28, "年末休暇"],
        [2017, 9999, 12, 29, "年末休暇"],
        [2017, 9999, 12, 30, "年末休暇"],
        [2017, 9999, 12, 31, "年末休暇"]
    ];
